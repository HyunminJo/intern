...
public class SampleController {
    ...
    public String sample(Model model) {
        ...
        /*
        model.addAttribute("adminList", this.adminService.getAdminList(new AdminDto()).getContent());
        model.addAttribute("adminGrpList", this.adminService.getAdminGrpUseList(0));
        model.addAttribute("menuList", this.ctgService.getCtgSubList(1, Constants.Y));
        model.addAttribute("ctgList", this.ctgService.getCtgSubList(2, Constants.Y));
        model.addAttribute("notiInfoList", this.ctgService.getCtgSubList(3, Constants.Y));
        model.addAttribute("faqList", this.ctgService.getCtgSubList(ApiConstants.CtgSeq.FAQ, Constants.Y));
        */
    ...
    public String boardForm(Model model, BoardArticle boardArticle) {
        ...
        if (StringUtils.equals(Constants.Mode.UPDATE, boardArticle.getMode())) {
            ...
            if (data == null) { throw new BadRequestException(Constants.ExMsg.DATA_EMPTY); }
