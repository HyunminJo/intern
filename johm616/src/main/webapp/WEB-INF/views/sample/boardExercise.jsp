...
<div class="table-form-wrap">
	<form name="searchForm"
		action=${popupYn eq 'Y' ? "/sample//sample/exerciseBoard/popup" : "/sample/exerciseBoard"}
		method="get">
		<table class="table-form">
			...
			<tbody>
				<tr>
					...
					<th>작성자</th>
					<td class="tl form-100p"><input type="text" name="wrtrName"
						value="<c:out value='${paramMap.wrtrName}'/>" /></td>
				</tr>
				<tr>
					<th>내용</th>
					<td class="tl form-100p" colspan="3"><input type="text"
						name="boardArtclCont"
						value="<c:out value='${paramMap.boardArtclCont}'/>" /></td>
				</tr>
				<tr>
					<th>등록일자</th>
					<td class="tl form-100p" colspan="3"><tiles:insertDefinition
							name="boDateSelect" /></td>
				</tr>
			</tbody>
		</table>
...
<div class="table-list-wrap">
	...
	<table class="table-list">
			...
			<tr>
				<c:if test="${popupYn eq 'Y' && paramMap.multYn eq 'Y'}">
					<th scope="col"><input type="checkbox"
						onclick="checkAll(this, 'chkBoard');" /></th>
				</c:if>
		...
		<tbody>
			...
			<c:forEach items="${list.content}" var="board" varStatus="status">
				<tr>
					<c:if test="${popupYn eq 'Y' && paramMap.multYn eq 'Y'}">
						<td><input type="checkbox" name="chkBoard"
							value="<c:out value='${status.index}'/>" /></td>
					</c:if>
					<c:if test="${popupYn eq 'Y' && paramMap.multYn eq 'N' }">
						<td>
							<button type="button" class="btn btn-primary"
								onclick="selectBoard(${status.index})">선택</button>
						</td>
					</c:if>
					<td><c:out
							value="${list.totalElements - ((list.number - 1) * list.size) - status.index }" />
					</td>
					<td><c:choose>
							<c:when test="${popupYn ne 'Y'}">
								<a
									href="/sample/exBoardFormMod?boardArtclSeq=${board.boardArtclSeq}&searchData=<core:param encode="true" />"><c:out
										value='${board.boardArtclTitle}' /></a>
							</c:when>
							<c:otherwise>
								<c:out value="${board.boardArtclTitle}" />
							</c:otherwise>
						</c:choose></td>
					<td><c:out value="${board.wrtrName}" /></td>
					<td><fmt:formatDate value="${board.mdfDtm}"
							pattern="yyyy-MM-dd kk:mm:ss" /></td>
					<td><fmt:formatDate value="${board.rgstDtm}"
							pattern="yyyy-MM-dd kk:mm:ss" /></td>
	...
	<div class="table-footer">
		...
		<c:if test="${popupYn ne 'Y' }">
			<div class="btn-wrap">
				<button type="button" class="btn btn-primary"
					onclick="location.href='/sample/exBoardFormReg';">등록</button>
			</div>
		</c:if>
	</div>
</div>
...
function searchKeywordSetEvent() {
	$("#serachKeyword").on('keyup', function() {
