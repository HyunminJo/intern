		...
		<table class="table-form">
			...
			<tbody>
				...
				<tr>
					...
					<td class="tl form-100p" colspan="3"><input type="text"
						name="wrtrName" value='<c:out value="${data.wrtrName}"/>'
		...
		<div class="table-footer">
			<div class="btn-wrap">
				...
				<button type="button" class="btn btn-cancel list"
					onclick="location.href='/sample/freeBoardList';">목록</button>
			</div>
		</div>
	</form>
</div>
...
<script>
	...
	function saveCont() {
		...
		$.ajax({
			url : param.mode == "create" ? "/sample/freeBoardCreate"
					: "/sample/freeBoardUpdate",
			type : "post",
			dataType : "json",
			data : param,
			traditional : true,
			success : function(data) {
				...
				if (param.mode == "create") {
					alert("등록하였습니다.");
					location.href = "/sample/freeBoardList";

				} else {
					alert("수정하였습니다.");
					location.href = "/sample/freeBoardFormMod?boardArtclSeq="
							+ param.boardArtclSeq;
				}
			},
	...
	function deleteCont() {
		...
		$.ajax({
			url : param.mode == "delete" ? "/sample/freeBoardDelete"
					: "/sample/freeBoardFail",
			type : "post",
			dataType : "json",
			data : param,
			traditional : true,
