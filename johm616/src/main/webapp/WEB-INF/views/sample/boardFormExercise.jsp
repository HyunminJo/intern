    ...
    <form name="form" id="saveForm" onsubmit="return false;">
        ...
        <div class="table-footer">
            <div class="btn-wrap">
                    ...
                    <button type="button" class="btn btn-primary" onclick="deleteCont()">삭제</button>
                    <button type="button" class="btn btn-cancel list" onclick="location.href='/sample/boardExercise';">목록</button>
            </div>
        </div>
    </form>
</div>
...
function saveCont() {
		...
		$.ajax({
			url         : param.mode == "create" ? "/sample/exBoardCreate" : "/sample/exBoardUpdate",
			type        : "post",
			dataType    : "json",
			data        : param,
			traditional : true,
			success     : function(data) {
				...
				if(param.mode == "create") {
					alert("등록하였습니다.");
					location.href="/sample/exerciseBoard";
				} else {
					alert("수정하였습니다.");
					location.href="/sample/exBoardFormMod?boardArtclSeq=" + param.boardArtclSeq;
				}
			},
...
function deleteCont() {
	...
	$.ajax({
		url          : param.mode == "delete" ? "/sample/exBoardDelete" : "/sample/exBoardFail",
		type         : "post",
		dataType     : "json",
		data         : param,
		traditional  : true,
