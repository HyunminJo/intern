...
<div class="table-form-wrap">
    <form name="searchForm" action=${popupYn eq 'Y' ? "/sample/searchBrand/popup" : "/sample/searchBrand"} method="get">
            ...
            <c:forEach items="${list.content}" var="board" varStatus="status">
                <tr>
                    ...
                    <c:if test="${popupYn eq 'Y' && paramMap.multYn eq 'N' }">
                        <td>
                            <button type="button" class="btn btn-primary" onclick="selectBoard(${status.index})">선택</button>
                        </td>
                    </c:if>
                    <td>
